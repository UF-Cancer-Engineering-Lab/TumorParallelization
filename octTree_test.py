...
class TestOctTree(unittest.TestCase):
    ...
    def isValidGPUOctTree(self, particles):
        GPUBufferSizeNodes = 100000  # estimateTreeSizeFromLeafCount(len(particles.index)) staying on the safe side with large buffer
    ...
    def test_10_mediumInitialSphere(self):
        particles = getInitialSphere(sphereRadius=10, particlesNumber=100)
        numOfIterations = 100
        print("Number of particles in test case", len(particles.index))
    ...
    def test_11_largeInitialSphere(self):
        particles = getInitialSphere(sphereRadius=20, particlesNumber=100)
        numOfIterations = 1
        print("Number of particles in test case", len(particles.index))
        for i in range(numOfIterations):
            isValidOctTree = self.isValidGPUOctTree(particles)
